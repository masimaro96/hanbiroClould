                ...
                <Row>
                    ...
                    <Col md={12} lg={9} className="right-content">
                        ...
                        <Grid container spacing={3}>
                            <Grid item xs={12} sm={6}>{CardBox("쿠버네티스 클러스터 기반 Cassandra 호환 NoSQL 데이터베이스 엔진", "ScyllaDB는 Cassandra와 호환되며, Cassandra 에 비해 수배이상 빠른 속도를 보장합니다. ScyllaDB는 C++기반의 Seastar 프레임워크로 개발됐습니다. ScyllaDB Cluster, Node, Shard로 구성되어, Scale out (확장)을 효율적으로 운영할 수 있습니다.")}</Grid>
                            ...
                            <Grid item xs={12}>{Imagecloudcompute("https://www.hanbiro.com/cloud/images/concept/scylla_cluster.gif")}</Grid>
                            <Grid item xs={12} sm={6}>{CardBox("컨테이너(POD) 하드웨어의 자유로운 구성으로 최적의 시스템 구성","컨테이너(POD)에 할당되는 서버자원은 고객의 요구에 맞게 구성할 수 있으며, Kubernetes ScyllaDB의 컨테이너(POD) 구성방식은 고객의 요구에 맞게 자유롭게 구성할 수 있습니다.")}</Grid>
                            <Grid item xs={12} sm={6}>{CardBox("자동 Failover 로 서비스 안정성확보","Kubernetes ScyllaDB의 컨테이너(POD)는 단위서비스별 3개의 replica 노드로 구성되며, 각각의 replica 컨테이너(POD)는 쿠버네티스의 다수의 워커노드에 분산배치되어 있습니다.")}</Grid>
