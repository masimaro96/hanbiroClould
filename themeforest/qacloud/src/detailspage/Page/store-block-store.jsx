                ...
                <Row>
                    ...
                    <Col md={12} lg={9} className="right-content">
                        ...
                        <Grid container spacing={3}>
                            <Grid item xs={12} sm={6}>{CardBox("한비로 Cloud 서버의 저장용량을 실시간확장", "데이터를 일정한 크기의 블록으로 나누어 저장 하는 방식이며, 한비로 Cloud 관리자콘솔에서 생성후 수분내에 볼륨을 사용할 수 있습니다. 한비로 Cloud 의 블록 Storage 에서 생성된 볼륨은 운영체제의 제한사항이 없습니다.")}</Grid>
                            <Grid item xs={12} sm={6}>{CardBox("스냅샷기능을 제공하며, 3 Replica 블록 Storage 운영","스냅샷 기능을 제공하며, 볼륨의 백업및 복제와 복원시에 사용됩니다. 블록 Storage 의 볼륨을 3 replica SET 으로 운영합니다.")}</Grid>
                            <Grid item xs={12}>{Imagecloudcompute("https://www.hanbiro.com/cloud/images/concept/hanbiro_storage.gif")}</Grid>
                            <Grid item xs={12} sm={6}>{CardBox("블록 Storage 의 주요제한사항","생성된 볼륨은 한비로 Cloud 플랫폼내의 가상서버(인스턴스)에서만 사용할 수 있습니다. 블록 Storage 에서 생성된 볼륨은 2개이상의 가상서버(인스턴스)에서 엑세스 할 수 없습니다. 최대볼륨 이상의 볼륨필요시 한비로 Cloud 컨설턴트에게 문의해주세요.")}</Grid>
                        </Grid>
