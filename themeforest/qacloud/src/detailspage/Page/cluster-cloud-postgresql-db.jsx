                ...
                <Row>
                    ...
                    <Col md={12} lg={9} className="right-content">
                        ...
                        <Grid container spacing={3}>
                            <Grid item xs={12} sm={6}>{CardBox("Failover 기능추가, 고가용성 I/O 분산 데이터베이스구현", "PostgreSQL 과 pg_auto_failover 를 기능을 이용한 Read 노드 자동확장 시스템을 구현합니다. PostgreSQL Read 컨테이너(POD)는 사용량에 따라 자동으로 증감되어 I/O 분산을 구현합니다. PostgreSQL 노드의 장애시 Monitor 노드가 장애처리를 진행하며, Primary 노드 컨테이너 장애에도 Monitor 노드에 의해서 Read Node 가 역활을 대행합니다.")}</Grid>
                            ...
                            <Grid item xs={12}>{Imagecloudcompute("https://www.hanbiro.com/cloud/images/concept/postgresql_cluster.gif")}</Grid>
                            <Grid item xs={12} sm={6}>{CardBox("컨테이너(POD) 하드웨어의 자유로운 구성으로 최적의 시스템 구성","컨테이너(POD)에 할당되는 서버자원은 고객의 요구에 맞게 구성할 수 있으며, Kubernetes PostreSQL의 컨테이너(POD)구성방식은 고객의 요구에 맞게 자유롭게 구성할 수 있습니다.")}</Grid>
                            <Grid item xs={12} sm={6}>{CardBox("자동 Failover 로 서비스 안정성확보","Kubernetes PostgreSQL 컨테이너(POD)는 단위서비스별 3개의 replica 노드로 구성되며, 각각의 replica 컨테이너(POD)는 쿠버네티스의 다수의 워커노드에 분산배치되어 있습니다.")}</Grid>
