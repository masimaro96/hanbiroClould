            ...
            <Container>
                <Row>
                    ...
                    <Col md={12} lg={9}>
                        ...
                        {Contentcloudcompute("클라우드기반 이벤트 및 데이터 스트리밍 플랫폼", "","쿠버네티스 클러스터의 샤딩과 Replica 를 지원하는 검색엔진","","Apache Kafka 엔진의 쿠버네티스 기반 클러스터","데이터 스트리밍 플랫폼입니다. 한비로 Cloud 플랫폼의","가상서버(인스턴스)와 연동한 클러스터 구축을","지원합니다.","","컨테이너(POD)의 장애발생시 대기상태의","컨테이너에서 서비스를 유지하는 무정지시스템을","구현합니다. Kfaka는 샤딩을 통해 수평확장을","지원하며, 토픽로그를 분산된 컨테이너로 분할 저장합니니다. 한비로의 모든 소프트웨어에서 운영중인","실시간 데이터 스트리밍 엔진입니다.")}
                        {Imagecloudcompute("https://www.hanbiro.com/cloud/images/concept/kafka_cluster.gif")}
                        {BoxCard("고성능의 Persistant Volume 운영, I/O 최적의 성능구현","SSD 기반의 PV 볼륨으로 운영되고 있으며, 3개의 replica 컨테이너(POD)가 하나의 서비스단위입니다. 쿠버네티스 워커노드의 하드웨어 장애시에도 타 워커노드의 replica 컨테이너(POD)에서 서비스가 진행됩니다.","하드웨어장애시 타 워커노드에 컨테이너(POD) 자동으로 재배치 무정지시스템구현","Kafka Data Streamming 엔진의 컨테이너(POD)는 단위서비스별 3개의 replica 노드로 구성되며, 각각의 replica 컨테이너(POD)는 쿠버네티스의 다수의 워커노드에 분산배치되어 있습니다.","자동으로 스케일아웃하여 컨테이너 확장, 서비스의 부하분산 구현","CPU 사용률, 메모리 사용률, 또는 다른 커스텀 메트릭 기준으로 컨테이너(POD) 수량을 주기적으로 조정(증감)하며, 서비스 확장을 위한 하드웨어 자원을 확보할 필요없이 서비스를 구동할 수 있습니다. 서비스의 부하분산및 비용을 효율적으로 사용할 수 있는 클러스터 솔루션입니다.","컨테이너(POD) 하드웨어의 자유로운 구성으로 최적의 시스템 구성","컨테이너(POD)에 할당되는 서버자원은 고객의 요구에 맞게 구성할 수 있으며, Kafka Data Streamming 엔진의 구성방식은 고객의 요구에 맞게 자유롭게 구성할 수 있습니다.")}
