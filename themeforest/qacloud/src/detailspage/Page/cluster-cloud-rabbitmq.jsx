                ...
                <Row>
                    ...
                    <Col md={12} lg={9} className="right-content">
                        ...
                        <Grid container spacing={3}>
                            <Grid item xs={12} sm={6}>{CardBox("메세지를 신뢰성있게 전달 할 수 있는 데이터 스트리밍 플랫폼", "AMQP 기반의 오픈소스 메세지 지향형 미들웨어입니다. RabbitMQ 서비스를 통해 신뢰성있는 메세지의 전달, 비동기처리 및 작업 Queue를 구현할 수 있습니다.")}</Grid>
                            ...
                            <Grid item xs={12}>{Imagecloudcompute("https://www.hanbiro.com/cloud/images/concept/rabbitmq_cluster.gif")}</Grid>
                            <Grid item xs={12} sm={6}>{CardBox("자동 Failover 로 서비스 안정성확보","쿠버네티스 워커노드별로 분산된 replica 컨테이너(POD)로 서비스 단위가 운영되고 있으며, 서비스중인 컨테이너(POD)의 장애시 대기중인 타 워커노드의 컨테이너(POD)에서 서비스가 운영됩니다.")}</Grid>
                            <Grid item xs={12} sm={6}>{CardBox("자동으로 스케일아웃(Autoscaling)하여 컨테이너 확장, 서비스의 부하분산 구현","CPU 사용률, 메모리 사용률, 또는 다른 커스텀 메트릭 기준으로 컨테이너(POD) 수량을 주기적으로 조정(증감)하며, 하드웨어 자원을 확보할 필요없이 서비스를 구동할 수 있습니다. 서비스의 부하분산및 비용을 효율적으로 사용할 수 있는 목적에 부합됩니다.")}</Grid>
                            <Grid item xs={12} sm={6}>{CardBox("컨테이너(POD) 하드웨어의 자유로운 구성으로 최적의 시스템 구성","컨테이너(POD)에 할당되는 서버자원은 고객의 요구에 맞게 구성할 수 있으며, RabbitMQ MiddleWare 엔진의 구성방식은 고객의 요구에 맞게 자유롭게 구성할 수 있습니다.")}</Grid>
                        </Grid>
