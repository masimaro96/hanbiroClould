...
function ClusterClickHouseTest(){
                ...
                <Row>
                    ...
                    <Col md={12} lg={9} className="right-content">
                        ...
                        <Grid container spacing={3}>
                            <Grid item xs={12} sm={6}>{CardBox("실시간 온라인분석처리(OLAP) 를 위한 오픈소스 컬럼 기반 DBMS", "데이터는 사용 가능한 노드 에 기록 된후 모든 노드에 복재본이 배포됩니다. 페타 바이트 규모의 데이터를 저장하고 처리 할 수 있습니다. 전통적인 DBMS 보다 100배이상 빠르고, 실시간 온라인 데이터 분석에 사용됩니다.")}</Grid>
                            <Grid item xs={12} sm={6}>{CardBox("데이터를 Shard에 분산저장하며, 비동기 멀티 마스터 지원","Multi-master 비동기 복제를 지원하며, 복제된 임의의 노드에서 데이터수집합니다. 데이터를 Shard 에 분산저장하며, 연산결과를 병렬로 실행하며 병합합니다.")}</Grid>
                            <Grid item xs={12}>{Imagecloudcompute("https://www.hanbiro.com/cloud/images/concept/clickhouse.jpeg")}</Grid>
                            <Grid item xs={12} sm={6}>{CardBox("컨테이너(POD) 하드웨어의 자유로운 구성으로 최적의 시스템 구성","컨테이너(POD)에 할당되는 서버자원은 고객의 요구에 맞게 구성할 수 있으며, Kubernetes ClickHouse의 컨테이너(POD) 구성방식은 고객의 요구에 맞게 자유롭게 구성할 수 있습니다.")}</Grid>
...
export default ClusterClickHouseTest;